...
describe('DocsPage', () => {
  ...
  // (Updated in v2.46.2 — MOBILE-4: 5+More nav, collapsible WNB, greeting truncation fix.)
  it('RS-26: Release Notes contains the latest v2.46.2 entry', async () => {
    const w = mountWith(DocsPage);
    await nextTick();
    await w.findAll('.docs-nav-btn').find(b => b.text().includes('Release Notes'))!.trigger('click');
    await nextTick();
    expect(w.find('.docs-section').text()).toContain('v2.46.2');
    w.unmount();
  });
