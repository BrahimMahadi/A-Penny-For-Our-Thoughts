...
describe('DocsPage', () => {
  ...
  // (Updated in v2.47.0 — MOBILE-5: installable PWA, monogram icon, tactile feedback.)
  it('RS-26: Release Notes contains the latest v2.47.0 entry', async () => {
    const w = mountWith(DocsPage);
    await nextTick();
    await w.findAll('.docs-nav-btn').find(b => b.text().includes('Release Notes'))!.trigger('click');
    await nextTick();
    expect(w.find('.docs-section').text()).toContain('v2.47.0');
    w.unmount();
  });
