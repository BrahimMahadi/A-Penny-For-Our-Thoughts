...
describe('DocsPage', () => {
  ...
  // (Updated in BUG-023/024 to assert the new v2.23.0 entry.)
  it('RS-26: Release Notes contains the latest v2.23.0 entry', async () => {
    const w = mountWith(DocsPage);
    await nextTick();
    await w.findAll('.docs-nav-btn').find(b => b.text().includes('Release Notes'))!.trigger('click');
    await nextTick();
    expect(w.find('.docs-section').text()).toContain('v2.23.0');
    w.unmount();
  });
  ...
  it('RS-26: Release Notes contains every shipped v2.x version (regression guard)', async () => {
    ...
    const versions = [
      'v2.23.0', 'v2.22.0', 'v2.21.0', 'v2.20.1', 'v2.20.0', 'v2.19.1', 'v2.19.0', 'v2.18.0',
  ...
  it('RS-26: Release Notes mentions each major redesign sprint (RS-9 through RS-25)', async () => {
    ...
    for (const sprint of ['BUG-023', 'BUG-024', 'RS-31', 'RS-30', 'RS-29', 'RS-28', 'RS-27', 'RS-25', 'RS-24', 'RS-23', 'RS-22', 'RS-21', 'RS-20', 'RS-19', 'RS-11', 'RS-1 through RS-9']) {
