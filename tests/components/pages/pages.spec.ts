...
describe('DocsPage', () => {
  ...
  // (Updated in v2.46.0 — mobile theme toggle: Settings Appearance panel + header icon.)
  it('RS-26: Release Notes contains the latest v2.46.0 entry', async () => {
    const w = mountWith(DocsPage);
    await nextTick();
    await w.findAll('.docs-nav-btn').find(b => b.text().includes('Release Notes'))!.trigger('click');
    await nextTick();
    expect(w.find('.docs-section').text()).toContain('v2.46.0');
    w.unmount();
  });
