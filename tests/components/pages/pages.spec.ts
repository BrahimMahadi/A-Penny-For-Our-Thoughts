...
describe('DocsPage', () => {
  ...
  // (Updated in v2.46.1 — MOBILE-3: iOS zoom fix, 0.72rem text floor, type-scale tokens.)
  it('RS-26: Release Notes contains the latest v2.46.1 entry', async () => {
    const w = mountWith(DocsPage);
    await nextTick();
    await w.findAll('.docs-nav-btn').find(b => b.text().includes('Release Notes'))!.trigger('click');
    await nextTick();
    expect(w.find('.docs-section').text()).toContain('v2.46.1');
    w.unmount();
  });
