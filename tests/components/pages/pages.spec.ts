...
describe('DocsPage', () => {
  ...
  // (Updated in v2.46.3 — BUG-037: Web Storage shim restores the jsdom test environment.)
  it('RS-26: Release Notes contains the latest v2.46.3 entry', async () => {
    const w = mountWith(DocsPage);
    await nextTick();
    await w.findAll('.docs-nav-btn').find(b => b.text().includes('Release Notes'))!.trigger('click');
    await nextTick();
    expect(w.find('.docs-section').text()).toContain('v2.46.3');
    w.unmount();
  });
